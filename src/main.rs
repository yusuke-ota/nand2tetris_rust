    ...
    while parser.has_more_commands() {
        ...
        match command_type {
            CommandType::ACommand(_) | CommandType::LCommand(_) => match parser.symbol().unwrap() {
                Symbol::Address(num) => write_string.push_str(&(num.to_string())),
                Symbol::Symbol(string) => write_string.push_str(&string)
            },
            CommandType::CCommand(_) => {
                let comp = comp(parser.comp().unwrap_or(CompType::Zero)).iter().map(|&iter| iter.to_string()).collect::<String>();
                let dest = dest(parser.dest().unwrap_or(DestType::Null)).iter().map(|&iter| iter.to_string()).collect::<String>();
                let jump = jump(parser.jump().unwrap_or(JumpType::Null)).iter().map(|&iter| iter.to_string()).collect::<String>();
                let add_string = comp.chars().chain(dest.chars()).chain(jump.chars()).collect::<String>();
